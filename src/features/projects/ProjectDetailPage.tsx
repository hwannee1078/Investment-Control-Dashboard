...
export default function ProjectDetailPage() {
  ...
  return <main className="page-shell">
    ...
    <section className="detail-panel" aria-labelledby="basic-info-title"><h2 id="basic-info-title">사업 기본 정보</h2><dl className="project-facts">
      <div><dt>소재</dt><dd>{project.material}</dd></div><div><dt>지역</dt><dd>{project.location}</dd></div><div><dt>현재 단계</dt><dd>{project.status}</dd></div>
      <div className="project-facts__investment"><dt>승인투자비</dt><dd>{project.approvalBudget === null ? '-' : `${currency.format(project.approvalBudget)}원`}</dd><small>누적투자비 {currency.format(summary.cumulativeTotal)}원 · 집행률 {summary.executionRate === null ? '-' : `${summary.executionRate.toFixed(1)}%`}</small></div>
    </dl></section>
    ...
    <section className="detail-panel rolling-chart-panel" aria-labelledby="rolling-chart-title"><h2 id="rolling-chart-title">분기별 Rolling Plan 비교</h2>
      {years.length === 0 ? <p className="empty-state">등록된 Rolling Plan 또는 실적이 없습니다.</p> : years.map((year) => <section key={year} className="rolling-year" aria-label={`${year}년 Rolling Plan`}><h3>{year}년</h3><div className="rolling-chart" role="group" aria-label={`${year}년 분기별 계획·실적 투자비 비교`}>
        {[1, 2, 3, 4].map((quarter) => { const quarterMonths = monthsForQuarter(year, quarter); const plan = sum(quarterMonths); const actual = sum(quarterMonths, true); const max = Math.max(1, ...[1, 2, 3, 4].map((q) => Math.max(Math.abs(sum(monthsForQuarter(year, q))), Math.abs(sum(monthsForQuarter(year, q), true))))); const key = `${year}-Q${quarter}`; return <button key={key} type="button" className="rolling-bar-group" disabled={year !== currentYear} aria-label={`${year}년 ${quarter}분기`} onClick={() => { setSelectedQuarter(key); setSelectedMonth(null) }}><span className="rolling-month">{quarter}분기</span><span className="rolling-bars"><i className="rolling-bar rolling-bar--plan" style={{ height: `${Math.max(2, Math.abs(plan) / max * 100)}%` }} /><i className="rolling-bar rolling-bar--actual" style={{ height: `${Math.max(2, Math.abs(actual) / max * 100)}%` }} /></span><small>계획 {Math.round(plan / 100000000)} / 실적 {Math.round(actual / 100000000)}억원</small></button> })}
      </div></section>)}
      {selected && <div className="rolling-month-detail" aria-label={`${selectedQuarter} 월별 상세`}><h3>{selectedQuarter} 월별 상세</h3><div className="rolling-chart" role="group" aria-label="월별 계획·실적 투자비 비교">{selectedMonths.map((month) => { const plan = value(month); const actual = value(month, true); const max = Math.max(1, ...selectedMonths.map((item) => Math.max(Math.abs(value(item)), Math.abs(value(item, true))))); return <button key={month} type="button" className="rolling-bar-group" onClick={() => setSelectedMonth(month)}><span className="rolling-month">{month}</span><span className="rolling-bars"><i className="rolling-bar rolling-bar--plan" style={{ height: `${Math.max(2, Math.abs(plan) / max * 100)}%` }} /><i className="rolling-bar rolling-bar--actual" style={{ height: `${Math.max(2, Math.abs(actual) / max * 100)}%` }} /></span><small>계획 {Math.round(plan / 100000000)} / 실적 {Math.round(actual / 100000000)}억원</small></button> })}</div></div>}
      {selectedMonth && <p className="rolling-reason"><strong>{selectedMonth} 차이 사유:</strong> {project.rollingPlan?.[selectedMonth]?.reason ?? '입력된 사유가 없습니다.'}</p>}
    </section>
  </main>
}
