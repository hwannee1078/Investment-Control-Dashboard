...
function errorMessage(error: unknown): string {
  ...
  return '대시보드 분석을 처리하지 못했습니다. 잠시 후 다시 시도해 주세요.'
}
...
export default function FloatingSafetyChatbot() {
  ...
  return <div className={`floating-safety-chatbot${open ? ' is-open' : ''}`}>
    {open ? <section className="floating-chat-panel" aria-label="통합 AI Agent 대화">
      <header><div><strong>통합 AI Agent</strong><span>승인된 문서와 대시보드 데이터를 근거로 답합니다.</span></div><button type="button" aria-label="AI Agent 닫기" onClick={() => setOpen(false)}>×</button></header>
      <div className="floating-chat-content" aria-live="polite">
        {answer ? <>
          <p>{answer.answer}</p>
          {answer.citations.length ? <div className="floating-citations">{answer.citations.map((citation) => <a key={`${citation.title}-${citation.section ?? ''}-${citation.url}`} href={citation.url} target="_blank" rel="noreferrer">{citation.title} · {citation.section ?? '관련 항목'}</a>)}</div> : null}
        </> : <p className="floating-chat-placeholder">안전규정·투자비·일정 질문을 자연어로 입력해 보세요.</p>}
      ...
      <small>근거가 없으면 추정하지 않습니다. 답변은 참고용으로 확인해 주세요.</small>
    </section> : null}
